...
```
现在我点击响应都有了，怎么让用户和机器走棋呢？我这里每一个棋子都有自己的类，来判断是否符合规则move，但是，在用户界面上是否能真的移动,怎么移动呢？用户（黑棋）走棋有两种情况，一是终点位置是空的，直接走；二是终点位置有对方的棋，"吃子,"吃子"还有一种特殊的情况，就是**将军**，棋局结束。这里开始我遇到的问题是：我更新棋子imgv的时候，用户体验到的是棋子直接从起点到终点，没有任何过程，这样效果就不行了。我现在是给棋子移动更新添加了一个动画过程：用户可以看到棋子是从起点沿着棋盘方格对角线有轨迹的移过去。
```
